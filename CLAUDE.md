...
| Размер шрифта, интервал | БД (settings) | Глобальная настройка |
...
- Стиль аккордов: нет / фон / рамка / фон+рамка (цикл, серверная настройка)
- Многоколоночный текст: 1-2-3 колонки (per-device, localStorage)
- Интервал: настраиваемый, расширенный диапазон (0.8–2.5)
