...
| Версии сложности аккордов (архитектура согласована) | Планируется |
...
## Версии песни (планируется)

Одна песня → несколько вариантов (оригинал/упрощённые/барре). Архитектура согласована, не реализовано.

**Миграция (план):** Новая таблица `song_versions (id, song_id FK, label, chordpro, key, transpose, sort_order, timestamps)`. В `songs` добавляется `default_version_id`. `chordpro/key/transpose` переезжают в song_versions.

**Индивидуально для версии:** `chordpro`, `key`, `transpose`.
**Общее для всей песни:** `title`, `artist`, `tags[]`, `status`, `youtube_urls[]`, `youtube_labels[]`.

**UI:**
- Просмотр: табы сверху текста (как YouTube), URL `/song/:id?v=:versionId`
- Редактор: вкладки + "+ Новая версия", выбор default (⭐), удаление (≥1 остаётся)
- Удаление песни каскадом удаляет все версии

**Настройки отображения** (`chordStyle`, `showChords`, `useH`, цвета) — per-theme, не per-version.

Подробности см. в SPEC.md раздел 7.
